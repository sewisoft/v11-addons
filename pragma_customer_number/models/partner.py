...
class ResPartner(models.Model):
    ...
    def _get_customer_number(self):
        ir_defaults = self.env['ir.default']
        default_auto_customer_ref = ir_defaults.get('res.partner', 'default_auto_customer_ref')
        auto_customer_ref_by_account = ir_defaults.get('res.partner', 'auto_customer_ref_by_account')
        ...
        for partner in self:
            if partner.customer:
                if default_auto_customer_ref:
                    # Anpassung Equitania
                    if auto_customer_ref_by_account and default_account_receivable and \
                                    partner.property_account_receivable_id.code != default_account_receivable.code:
                        ...
                        if not partner.cust_ref_print or partner.cust_ref_print != partner.property_account_receivable_id.code:
                            self.write({'cust_ref_print': partner.property_account_receivable_id.code})
                    else:
                        ...
                        if not partner.cust_ref_print or partner.cust_ref_print != _cust_auto_ref:
                            self.write({'cust_ref_print': _cust_auto_ref})
                else:
                    ...
                    if not partner.cust_ref_print or partner.cust_ref_print != _cust_no_auto_ref:
                        self.write({'cust_ref_print': _cust_no_auto_ref})
    ...
    def _auto_customer_ref(self):
        default_auto_customer_ref = self.env['ir.default'].get('res.partner', 'default_auto_customer_ref')
        for partner in self:
            if partner.customer:
                if default_auto_customer_ref:
    ...
    # wird im View verwendet, um die Nummer nachträglich erzeugen zu können
    # (wenn z.B. ein bestehender Lieferant zum Kunden wird)
    ...
    def create_customer_number(self):
        ...
        for partner in self:
            ...
            # in allen anderen Fällen
                ...
                vals['cust_auto_ref'] = self.env['ir.sequence'].next_by_code('customer.number')
