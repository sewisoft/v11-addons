...
class EqSaleOrderExtension(models.Model):
    ...
    def onchange_partner_id(self):
        super(EqSaleOrderExtension, self).onchange_partner_id()
    ...
    def _prepare_invoice(self):
        ...
        :return:
        """
        result = super(EqSaleOrderExtension, self)._prepare_invoice()
    ...
    def _amount_all(self):
        ...
        :return:
        """
        super(EqSaleOrderExtension, self)._amount_all()
        for order in self:
            val1 = 0
            val = 0
...
class EqInfoOptional(models.TransientModel):
...
class EqSaleConfigurationAddress(models.TransientModel):
    _inherit = 'res.config.settings'

    def set_values(self):
        super(EqSaleConfigurationAddress, self).set_values()
        ir_default = self.env['ir.default']
        ir_config_obj = self.env['ir.config_parameter']

        ir_default.set('sale.order', 'default_show_address', self.default_show_address or False)
        ir_default.set('sale.order', 'default_search_only_company', self.default_search_only_company or False)
        ir_default.set('sale.order', 'show_delivery_date', self.default_show_delivery_date)
        ir_default.set('sale.order', 'use_calendar_week', self.default_use_calendar_week)
        ir_default.set('sale.order', 'eq_show_preview_button', self.default_eq_show_preview_button)
        ir_default.set('sale.order.line', 'eq_use_internal_description', self.default_eq_use_internal_description)
    ...
    def get_values(self):
        res = super(EqSaleConfigurationAddress, self).get_values()
        ir_default = self.env['ir.default']
        ir_config_obj = self.env['ir.config_parameter']

        notification = ir_default.get('sale.order', 'default_show_address')
        only_company = ir_default.get('sale.order', 'default_search_only_company')
        show_delivery_date = ir_default.get('sale.order', 'show_delivery_date')
        use_calendar_week = ir_default.get('sale.order', 'use_calendar_week')
        eq_use_internal_description = ir_default.get('sale.order.line', 'eq_use_internal_description')
        eq_show_preview_button = ir_default.get('sale.order', 'eq_show_preview_button')
    ...
    default_show_address = fields.Boolean(string='Show street and city in the partner search of the Sale and Purchase '
                                                 'Order [eq_sale]', help="This adds the street and the city to the "
                                                                         "results of the partner search of the Sale "
                                                                         "and Purchase Order.")
    default_search_only_company = fields.Boolean(string='Only Search for Companies [eq_sale]',
                                                 help="Only Companies will be shown in the Customer search of the "
                                                      "Sale and Purchase Order.")
    group_product_rrp = fields.Boolean(string='Show RRP for products [eq_sale]',
                                       implied_group='eq_sale.group_product_rrp')
    ...
    default_eq_use_internal_description = fields.Boolean('Use internal description for sale orders [eq_sale]',
                                                         help='The internal description will be used for sale orders '
                                                              'not the sale description',
...
class EqSaleOrderLine(models.Model):
    ...
    def product_id_change(self):
        ...
        """
        vals = super(EqSaleOrderLine, self).product_id_change()
        ...
        eq_use_internal_descriptionion = self.env['ir.default'].get('sale.order.line', 'eq_use_internal_description')
        attributes = product_id.attribute_value_ids

        # set product name only after first change of quantity -
        # it's our workaround for refresh problem after each change of quantity
