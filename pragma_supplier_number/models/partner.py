...
class ResPartner(models.Model):
    ...
    def _get_supplier_number(self):
        default_auto_supplier_ref = self.env['ir.default'].get('res.partner', 'default_auto_supplier_ref')
        auto_supplier_ref_by_account = self.env['ir.default'].get('res.partner', 'auto_supplier_ref_by_account')
        ...
        for partner in self:
            if partner.supplier:
                if default_auto_supplier_ref:
                    # Anpassung Equitania
                    if auto_supplier_ref_by_account and default_account_payable and \
                                    partner.property_account_payable_id.code != default_account_payable.code:
                        partner.supplier_number = partner.property_account_payable_id.code
                        if not partner.supp_ref_print or \
                                        partner.supp_ref_print != partner.property_account_payable_id.code:
                            partner.write({'supp_ref_print': partner.property_account_payable_id.code})
                    else:
                        ...
                        if not partner.supp_ref_print or partner.supp_ref_print != _supp_auto_ref:
                            partner.write({'supp_ref_print': _supp_auto_ref})
                else:
                    ...
                    if not partner.supp_ref_print or partner.supp_ref_print != _supp_no_auto_ref:
                        partner.write({'supp_ref_print': _supp_no_auto_ref})
    ...
    # (wenn z.B. ein bestehender Kunde zum Lieferanten wird)
    ...
    def create_supplier_number(self):
        ...
        for partner in self:
            ...
            # in allen anderen Fällen
                ...
                vals['supp_auto_ref'] = self.env['ir.sequence'].next_by_code('supplier.number')
